...
import {
  ...
  getContentManagerLessons,
  publishContentManagerLesson,
  unpublishContentManagerLesson,
...
export default function ContentManagerLessonsPage() {
  ...
  const [lessons, setLessons] = useState<ContentManagerLesson[]>(
    [],
  );
  ...
  useEffect(() => {
    ...
    async function loadLessons() {
      try {
        const result = await getContentManagerLessons(
          token,
          courseDocumentId,
        );
  ...
  async function handleTogglePublish(lesson: ContentManagerLesson) {
    const storedToken = getToken();
    if (storedToken === null) {
      router.push("/login");
      return;
    ...
    try {
      if (lesson.publishedAt) {
        await unpublishContentManagerLesson(storedToken, lesson.documentId);
      } else {
        await publishContentManagerLesson(storedToken, lesson.documentId);
      }
  ...
  async function handleDelete(
    ...
    try {
      await deleteContentManagerLesson(
        token,
        lessonDocumentId,
      );
    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <div className="mx-auto max-w-6xl px-6 py-8">
        <section className="flex flex-col gap-5 rounded-3xl bg-slate-950 p-8 md:flex-row md:items-center md:justify-between">
          ...
          <Link
            href={`/content-manager/courses/${courseDocumentId}/lessons/new`}
                ...
                >
                  <div className="flex flex-col gap-4 md:flex-row md:items-center">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Link
                        href={`/content-manager/courses/${courseDocumentId}/lessons/${lesson.documentId}`}
