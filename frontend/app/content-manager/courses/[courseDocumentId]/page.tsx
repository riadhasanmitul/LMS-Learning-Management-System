...
import {
  publishContentManagerCourse,
  unpublishContentManagerCourse,
...
export default function ContentManagerCoursePage() {
  ...
  async function handleDelete() {
    ...
    try {
      await api.delete(`/api/courses/${courseDocumentId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  async function handleTogglePublish() {
    ...
    try {
      if (course?.publishedAt) {
        await unpublishContentManagerCourse(storedToken, courseDocumentId);
        setCourse((curr) => (curr ? { ...curr, publishedAt: null } : curr));
        setSuccess("Course saved as draft (unpublished).");
      } else {
        await publishContentManagerCourse(storedToken, courseDocumentId);
        setCourse((curr) =>
        ...
        <section className="flex flex-col gap-4 rounded-3xl bg-slate-950 p-8 md:flex-row md:items-center md:justify-between md:p-10">
          <div>
            ...
            <p className="mt-3 text-sm text-slate-400">
              Manage course information, lessons, and quizzes.
            </p>
          </div>
